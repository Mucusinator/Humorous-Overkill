...
public class AudioManager : MonoBehaviour
{
    ...
    public List<AudioData> clips = new List<AudioData>();
    ...
    public class AudioFade
    {
        ...
        public bool Update(float delta)
        {
            ...
            // check if complete
            if (data.sources[index1].volume < maximum) return true;
            // update final volumes
            data.sources[index1].volume = maximum;
            data.sources[index2].volume = minimum;
            return false;
        }
    }
}