...
public class spawnTeddys : MonoBehaviour
{
    public List<GameObject> teddyPrefabs;
	...
	void Start ()
    {
        for(int x = 0; x < gridSize.x; x++)
        {
            for (int y = 0; y < gridSize.y; y++)
            {
                ...
                GameObject teddy = Instantiate(teddyPrefabs[Random.Range(0, teddyPrefabs.Count)], spawnPos, Quaternion.Euler(0, 90, 0) * Quaternion.LookRotation(-spawnPos, Vector3.up), transform);
